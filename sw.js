...
const CACHE_NAME = 'V3.4.176  更新日期：20260712';

// 更新日志（每次发布新版本时更新）
const UPDATE_LOGS = [
    'fix 修复模板URL图片同步失败：序列化降级处理中模板只保留id/name/items，丢失photos字段导致URL图片无法同步',
    ...
    'fix 修复撤销到指定位置内存统计错误：未区分快照和差异项，导致undoMemoryUsage可能变为负数',
    'fix 修复阶梯价格标签不随模式切换：面积模式下已添加的阶梯行仍显示"件"和"元/件"',
    'fix 修复数据导入功能漏导入字段：导入备份后签名/收藏/系统设置会丢失',
    'fix 修复默认税率值不一致：保存系统设置时清空税率会被错误设为13%，应为1%',
    'fix 修复编辑已有报价时重复增加客户订单统计',
    'fix 修复大写金额转换bug：亿位段后万位段全0时错误多出"万"字（如1亿被错读成"壹亿万"）',
    'fix 修复供应商报价列表排序后index错位bug：勾选"首选"或按价格排序后，点击编辑/删除/选用可能误操作到错误的供应商',
    'fix 修复供应商备注保存bug：产品库编辑产品时供应商报价的备注字段被错误存成"on"，备注内容无法保存',
    'fix 修复界面显示问题：团队功能（团队设置、团队活动、团队成员）移到 Gist 配置区域',

    'feat 云同步支持切换 Gist/Repository 两种同步方式',
